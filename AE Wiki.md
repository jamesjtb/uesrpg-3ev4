UESRPG 3ev4 — Active Effects Guide (Foundry VTT v13)

This document describes the Active Effects (AE) framework implemented for the UESRPG 3ev4 system in Foundry VTT v13.351.
...
Full provenance in TN breakdown (collapsible)
...
Is applied as part of the same damage workflow

Uses the correct damage type

Is reduced by resistances and toughness

Is shown in the damage breakdown
...
3.3 Damage types, armor & resistance
Armor Rating
...
Natural Toughness reduces all damage types

It functions like AR but does not count as armor

3.4 Damage card UI (locked)

Single chat card per hit

Compact summary by default

Collapsible breakdown (TN-style)

Provenance shown for:

Weapon base damage

Active Effects

Armor, resistance, toughness sources

Removed:

Redundant “Final” vs “Total”

Separate cards per damage type
...
Max values are derived

Current values are clamped only if exceeding max

No direct mutation of current values via AE
...
7.2 Encumbrance penalty lanes (RAW-aligned)
Test penalty
...
7.3 Encumbrance → Fatigue conversion (RAW)
...
Encumbrance overflow (derived)

Fatigue bonus AE

Fatigue level calculation

Base fatigue penalty
...
OVERRIDE replaces ADD for the same key

Deterministic resolution via priority
...
Armor mobility penalties as explicit AE lanes
(currently handled internally, not AE-exposed)
