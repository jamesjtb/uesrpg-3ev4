...
/**
   ...
   * For NPCs, uses magic profession as fallback if no magic skill exists.
   */
  async _castAttackSpell(spell, targets) {
    ...
    // Fallback to NPC magic profession or WP bonus if no skill found
    let baseTN = 0;
    if (magicSkill) {
      baseTN = Number(magicSkill.system?.value ?? 0);
    } else {
      // NPCs use magic profession as fallback
      const magicProf = Number(this.actor?.system?.professions?.magic ?? 0);
      if (magicProf > 0) {
        baseTN = magicProf;
      } else {
        // Final fallback: WP bonus
        const wpTotal = Number(this.actor?.system?.characteristics?.wp?.total ?? 0);
        baseTN = Math.floor(wpTotal / 10);
      }
    }
